...
import { ActivityIndicator, Alert, Image, Pressable, ScrollView, StyleSheet, Text, View, type ViewStyle } from 'react-native';
...
export function BrandHeader({ eyebrow, title, subtitle }: { eyebrow?: string; title: string; subtitle?: string }) {
  return <View style={styles.header}><Image accessible={false} source={require('../../assets/images/madger-logo-transparent.png')} style={styles.logo} /><View style={styles.headerCopy}>{eyebrow ? <Text style={styles.eyebrow}>{eyebrow}</Text> : null}<Text accessibilityRole="header" style={styles.title}>{title}</Text>{subtitle ? <Text style={styles.subtitle}>{subtitle}</Text> : null}</View></View>;
}

export function Card({ children, style }: PropsWithChildren<{ style?: ViewStyle }>) { return <View style={[styles.card, style]}>{children}</View>; }

export function SectionTitle({ children, action }: PropsWithChildren<{ action?: ReactNode }>) { return <View style={styles.sectionRow}><Text accessibilityRole="header" style={styles.sectionTitle}>{children}</Text>{action}</View>; }

export function ActionButton({ label, icon, onPress, secondary = false, disabled = false, hint, errorMessage = 'This action could not be completed. Please try again.' }: { label: string; icon: keyof typeof MaterialCommunityIcons.glyphMap; onPress: () => unknown | Promise<unknown>; secondary?: boolean; disabled?: boolean; hint?: string; errorMessage?: string }) {
  ...
  async function activate() {
    ...
    setBusy(true);
    try { await onPress(); }
    catch { Alert.alert('Unable to complete action', errorMessage); }
    finally { pending.current = false; setBusy(false); }
  }
  const color = secondary ? COLORS.cream : COLORS.background;
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityHint={hint} accessibilityState={{ disabled: disabled || busy, busy }} disabled={disabled || busy} onPress={activate} style={({ pressed }) => [styles.button, secondary && styles.buttonSecondary, disabled && styles.disabled, pressed && styles.pressed]}>{busy ? <ActivityIndicator color={color} /> : <MaterialCommunityIcons accessible={false} name={icon} size={20} color={color} />}<Text style={[styles.buttonLabel, secondary && styles.buttonLabelSecondary]}>{label}</Text></Pressable>;
}
