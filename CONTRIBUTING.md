...
6. Use only channel URLs listed in `README.md`. A Reddit community name or Discord invite must be verified in the authenticated platform before it is added to code, structured data, documentation, or campaign copy.
7. Update tests/checklists and `CHANGELOG.md` with the implementation.
8. Run `npm ci`, `git diff --check`, `npm run build`, `npm run check`, `npm run validate`, and `npm audit --omit=dev`; complete visual/accessibility QA as applicable.
9. Submit a reviewable commit/PR describing exact files, user impact, tests, warnings, unresolved decisions, and mint verification.
